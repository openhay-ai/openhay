...
- Preserve the original structure and formatting: headings, paragraphs, lists, tables,
  code blocks, inline code, links, and emphasis.
- Do NOT add commentary, summaries, citations, or explanations.
- If the content contains multiple languages, only translate parts that are not already
  in the target language; keep target-language spans as-is.
...
Output format:
- Return only the translated content, using markdown if the input appears to be markdown
  or HTML-derived text. Do not wrap with any extra prose.
"""
