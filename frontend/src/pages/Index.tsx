  ...
  useEffect(() => {
    const run = async () => {
      try {
        ...
        const serverKeywords: { keyword: string; count: number }[] =
          data.keywords || [];
        if (items.length > 0) {
          ...
          <section className="max-w-3xl mx-auto pb-40 min-h-[calc(100vh-10rem)] flex items-center">
            <div className="flex flex-col items-center w-full">
              ...
              <p className="text-center text-muted-foreground max-w-xl mb-10">
                OpenHay giúp bạn giải đáp mọi thắc mắc trong học tập và cập nhật
              ...
              <div className="w-full">
                ...
                >
                  <CarouselContent>
                    {(activeCat
                      ? filled.filter((f) => f.category === activeCat)
                      : filled
                    ).map((f) => (
                      <CarouselItem
                        key={f.title}
                      ...
                      >
                        <article
                          onClick={() =>
                            handleSubmit(`Tin tức mới nhất về ${f.title}`)
                          }
                          className={cn(
                ...
                <div className="flex flex-wrap gap-2">
                  {chips.map((c) => (
                    <Button
                      ...
                      variant="chip"
                      size="sm"
                      onClick={() => handleSubmit(`Tin tức mới nhất về ${c}`)}
                    >
                      {c}
                    </Button>
                  ))}
